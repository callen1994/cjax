...
export function useCJAX(serv, equalTestOverride, copyerOverride) {
    const getData = useMemo(() => {
        ...
        if (!equalTest || !copyer) {
            if (equalTest || copyer)
                console.warn("You passed a equal test, but not a copyer function. because a default wasn't provided to the function that you didn't pass the function you did pass is getting ignored...");
            return () => serv?.current();
        }
    ...
    return useSyncExternalStore(serv?.listen ||
        (() => {
            return () => { };
        }), getData);
}
...
// service.
pipeBuilder, // * usePipe and the Constructor
dependencies = [], opts) {
    const { test } = opts || {};
    ...
    useEffect(() => {
        ...
        if (!built)
            return; // if I'm building a pipe that relies on other pipes that might not be ready yet, then the pipeBuilder will be returning undefined while the dependencies are still undefined (and I have to make sure I'm adding the dependencies! so it gets built once they are ready)
...
export function usePipeHere(pipeBuilder, // * usePipe and the Constructor
reducerDeps = [], opts) {
